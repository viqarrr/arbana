            ...
            <div class="border-gray-200 flex min-w-0 flex-1 flex-col border-e">
                <div class="bg-white border-gray-200 flex flex-col p-4">
                    ...
                    <div class="border-gray-200 flex items-center justify-between border-b border-dashed pb-2 mb-4">
                        <h3 class="text-lg font-semibold text-gray-800">Kelola Informasi</h3>
                    </div>
                    ...
                    <div class="space-y-3">
                            <div class="bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md transition">
                                <div class="flex items-start justify-between">
                                    <div class="flex-1">
                                        <h4 class="font-semibold text-gray-900 mb-2">Deskripsi Footer</h4>
                                        <p class="text-sm text-gray-700 mb-4">
                                            {{ Str::limit($information->footer_description, 150) }}</p>

                                        <h4 class="font-semibold text-gray-900 mb-2">Peraturan Pendakian</h4>
                                        <div class="text-sm text-gray-700 prose prose-lg">{!! $information->hiking_rules !!}</div>
                                    </div>

                                    <div class="flex gap-2 ml-4">
                                        <button @click="$dispatch('open-modal', 'edit-information-{{ $information->id }}')"
                                            type="button"
                            ...
                            <!-- Edit Modal -->
                            <x-app.modal name="edit-information-{{ $information->id }}" title="Edit Informasi"
                                max-width="3xl">
                                <form action="{{ route('admin.information.update', $information->id) }}" method="POST">
                                    ...
                                    <div class="space-y-5">
                                        <div>
                                            ...
                                            <textarea name="footer_description" rows="3"
                                                ...
                                                placeholder="Tulis deskripsi untuk bagian footer..." required>{{ $information->footer_description }}</textarea>
                                        </div>
                                        ...
                                        <div class="border-gray-200 focus-within:ring-blue-200 overflow-hidden rounded-lg border focus-within:ring"
                                            ...
                                            <div x-ref="quillEditor" class="min-h-40">{!! old('hiking_rules', $information->hiking_rules) !!}</div>
                                            <input type="hidden" name="hiking_rules" x-ref="hiddenContent"
                                                value="{{ old('hiking_rules', $information->hiking_rules) }}">
                                        </div>
                                    ...
                                    <div class="mt-6 flex justify-end space-x-3">
                                        <button type="button"
                                            @click="$dispatch('close-modal', 'edit-information-{{ $information->id }}')"
                                        ...
                                        <button type="submit"
                                            ...
                                            onclick="saveQuillContent({{ $information->id }})">
                        ...
                        @foreach ($contacts as $contact)
                            <div class="bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md transition">
                                <div class="flex items-start justify-between">
                                    <div class="flex-1">
                                        <div class="flex items-center gap-3 mb-2">
                                            <div
                                                ...
                                                <i
                                                    class="fas fa-{{ $contact->label == 'Email' ? 'envelope' : ($contact->label == 'Phone' || $contact->label == 'WhatsApp' ? 'phone' : 'link') }} text-blue-600"></i>
                                            </div>
                                            <div>
                                    ...
                                    <div class="flex gap-2 ml-4">
                                        <button @click="$dispatch('open-modal', 'edit-contact-{{ $contact->id }}')"
                                            type="button"
                                            class="text-blue-600 hover:bg-blue-50 border border-blue-200 rounded-lg px-3 py-1.5 text-xs">
                                            <i class="fas fa-edit"></i> Edit
                                        </button>
                            ...
                            <x-app.modal name="edit-contact-{{ $contact->id }}" title="Edit Kontak" max-width="2xl">
                                <form action="{{ route('admin.contacts.update', $contact) }}" method="POST">
                                    ...
                                    <div class="mt-6 flex justify-end space-x-3">
                                        <button type="button"
                                            @click="$dispatch('close-modal', 'edit-contact-{{ $contact->id }}')"
                                            class="border-gray-300 bg-white text-gray-700 rounded-lg border px-4 py-2 text-sm">Batal</button>
                                        <button type="submit"
                                            class="bg-blue-600 text-white rounded-lg px-4 py-2 text-sm">
                                            <i class="fas fa-check mr-2"></i> Simpan
